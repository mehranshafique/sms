...
def load_env_file():
    """Load chafon.env from the same folder as this script (does not override existing env vars)."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    for name in ('chafon.env', '.env'):
        ...
        with open(path, encoding='utf-8') as handle:
            for raw_line in handle:
                ...
                if key and key not in os.environ:
                    ...
                    loaded += 1
        print(f"Loaded {loaded} setting(s) from {path}")
        return path
    return None
...
# --- CONFIGURATION (chafon.env + environment variables) ---
...
def start_server():
    if not HARDWARE_SECRET:
        print('WARNING: HARDWARE_SECRET is not set. Add it to chafon.env in this folder.')
