        ...
        <div class="row page-titles mx-0">
            <div class="col-sm-6 p-md-0">
                <div class="welcome-text">
                    <h4>{{ __('department.edit_department') }}</h4>
                </div>
            </div>
        </div>
                        ...
                        <form action="{{ route('departments.update', $department->id) }}" method="POST" id="departmentForm">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control" value="{{ $department->name }}" placeholder="{{ __('department.enter_name') }}" required>
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label class="form-label">{{ __('department.code') }}</label>
                                    <input type="text" name="code" class="form-control" value="{{ $department->code }}" placeholder="{{ __('department.enter_code') }}">
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="head_of_department_id" class="form-control default-select" data-live-search="true">
                                        ...
                                        @foreach($staff as $id => $name)
                                            <option value="{{ $id }}" {{ $department->head_of_department_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <button type="submit" class="btn btn-primary mt-3" id="submitBtn">{{ __('department.update') }}</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    $(document).ready(function() {
        ...
        $('#departmentForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                type: 'POST', // Method PUT is handled by _method field
                data: $(this).serialize(),
