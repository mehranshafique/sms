        ...
        <div class="row page-titles mx-0 mb-4 p-4 bg-white rounded shadow-sm align-items-center">
            <div class="col-sm-6 p-0">
                <div class="welcome-text">
                    <h4 class="text-primary fw-bold fs-20">{{ __('school_event.edit') }}</h4>
                    <p class="mb-0 text-muted fs-14">{{ $schoolEvent->name }}</p>
                </div>
            </div>
            <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('school-events.index') }}">{{ __('school_event.page_title') }}</a></li>
                    <li class="breadcrumb-item active">{{ __('school_event.edit') }}</li>
                </ol>
            </div>
        </div>

        <div class="alert alert-warning">
            <i class="fa fa-info-circle me-2"></i>{{ __('school_event.edit_rebuild_notice') }}
        </div>
