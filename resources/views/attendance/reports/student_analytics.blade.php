...
<div class="content-body">
    <div class="container-fluid">
        ...
        <div class="row page-titles mx-0 mb-4 pb-3 border-bottom">
            <div class="col-sm-5 p-md-0 d-flex align-items-center">
            ...
            <div class="col-sm-7 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                <form method="GET" class="d-flex align-items-center w-100 justify-content-end">
                    
                    @if($isSubjectWise)
                    <select name="subject_id" class="form-control default-select me-2" onchange="this.form.submit()" style="max-width: 250px;">
                        <option value="">-- {{ __('attendance.select_subject') }} --</option>
                        @foreach($subjects as $id => $name)
                            <option value="{{ $id }}" {{ $selectedSubjectId == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                    @endif
                ...
                <div class="col-12">
                    <div class="card shadow-sm">
                        <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                            <h4 class="card-title mb-0">{{ __('attendance.detailed_logs') }} ({{ __('attendance.period_' . $period) }})</h4>
                            @if($isSubjectWise && isset($subjects[$selectedSubjectId]))
                                <span class="badge badge-primary">{{ $subjects[$selectedSubjectId] }}</span>
                            @endif
                        </div>
                        <div class="card-body">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @elseif($log->status == 'excused') <span class="badge badge-info">{{ __('attendance.excused') ?? 'Excused' }}</span>
                                                @elseif($log->status == 'half_day') <span class="badge badge-secondary">{{ __('attendance.half_day') ?? 'Half Day' }}</span>
