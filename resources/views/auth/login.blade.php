...
{{--                                        </defs>--}}
{{--                                    </svg>--}}
                                    <img src="https://e-digitex.com/public/images/smsslogonew.png" alt="" style="width: 150px !important;">
                                </a>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            toggles.forEach(function(toggle) {
                // Optional explicit target via data-target (CSS selector)
                ...
                function findInput() {
                    ...
                    // fallback: search for an input in the same .position-relative or previous sibling
                    let container = toggle.closest('.position-relative') || toggle.parentElement;
                    if (!container) container = document;
                    return container.querySelector('input[type="password"], input[type="text"], input[type="email"]');
                }
                ...
                function updateIcons(isShown) {
                    // If you have two icons (eye and eye-slash) toggle their visibility
                    const eyeOn = toggle.querySelector('.fa-eye');
                    const eyeOff = toggle.querySelector('.fa-eye-slash');
                    if (eyeOn) eyeOn.style.display = isShown ? 'inline' : 'none';
                    if (eyeOff) eyeOff.style.display = isShown ? 'none' : 'inline';
                }

                // Set initial state (assume hidden)
                updateIcons(false);

                function togglePassword() {
                    const isText = input.type === 'text';
                    // preserve cursor position for supported inputs
                    let pos = null;
                    ...
                    toggle.setAttribute('aria-pressed', String(nowShown));
                    updateIcons(nowShown);
                }
