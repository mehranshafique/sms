...
<head>
    ...
    <title>{{ __('reports.transcript') }} (ESU) - {{ $student->full_name }}</title>
    <style>
        @page { margin: 10mm; }
        ...
        .semester-header { background: #e0e0e0; padding: 5px; font-weight: bold; border: 1px solid #000; font-size: 12px; }
        ...
        .summary-box { border: 2px solid #000; padding: 10px; margin-top: 10px; font-weight: bold; font-size: 11px; }
...
<body>
    ...
    <div class="header">
        <div style="font-size:11px; font-weight:bold; text-transform:uppercase;">{{ __('reports.epst_republic_line') }}</div>
        <div style="font-size:11px;">{{ __('reports.esu_ministry_line') }}</div>
        <div class="inst-name">{{ $student->institution->name }}</div>
        <div class="inst-address">{{ $student->institution->address }}</div>
        <div class="report-title">{{ __('reports.esu_transcript_title') }}</div>
    </div>
        ...
        @foreach($history as $sessionName => $semesters)
            ...
            @foreach($semesters as $semName => $data)
                <div class="session-block">
                    <div class="semester-header">{{ __('reports.semester') }} {{ $data['semester'] }}</div>
                    <table class="lmd-table">
                        ...
                        <tbody>
                            @foreach($data['units'] as $ue)
                                <tr class="ue-row">
                                    <td class="text-left"><span class="ue-code">{{ $ue['unit_code'] }}</span> - {{ $ue['unit_name'] }}</td>
                    ...
                    <div class="summary-box">
                        {{ __('lmd.credits_attempted') }}: {{ $data['credits_attempted'] }} |
                        {{ __('lmd.credits_earned') }}: {{ $data['credits_earned'] }} |
                        {{ __('lmd.average') }}: {{ $data['average'] }}/20 |
                        {{ __('lmd.mention') }}: {{ $data['mention'] }} |
                        {{ __('reports.decision') }}: <strong>{{ $data['decision'] }}</strong>
                    </div>
                </div>
            @endforeach
        @endforeach
    ...
    <div class="footer">{{ __('results.computer_generated') }} | {{ now()->format('d M, Y h:i A') }}</div>
</body>
</html>
