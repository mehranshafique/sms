...
use App\Services\Attendance\AttendanceGateStatusService;
...
class AttendanceApiController extends Controller
{
    public function __construct(
        ...
        protected AttendanceScheduleResolver $scheduleResolver,
        protected AttendanceGateStatusService $gateStatusService
    ) {}
    ...
    private function processStudentAttendance($student, $date, $time, $scanTime, $method)
    {
        ...
        $isSubjectWise = $this->gateStatusService->isSubjectWiseInstitution($institutionId);

        $resolvedSchedule = $this->scheduleResolver->resolveForClassSection(
            $institutionId,
            (int) $enrollment->class_section_id
        );

        $gateContext = $this->gateStatusService->resolve(
            (int) $enrollment->class_section_id,
            $institutionId,
            $scanTime,
            $resolvedSchedule
        ...
        if (!$attendance) {
            ...
            $checkInTime = $this->gateStatusService->combineDateAndTime(
                Carbon::parse($date),
                $attendance->check_in
            );
    ...
    {
        return $this->gateStatusService->combineDateAndTime($day, $timeValue);
    }
    ...
    {
        return $this->gateStatusService->parseTimeOfDay($timeValue);
    }
